...
int main(int argc, char *argv[]) {
  ...
  if (0 != MPI_Init(&argc, &argv))
    return EXIT_FAILURE;

  int size, rank;
  if (0 != MPI_Comm_size(MPI_COMM_WORLD, &size))
    return EXIT_FAILURE;
  ...
  unsigned init_seed =
      cfg.seed == 0 ? (unsigned)time(NULL) : (unsigned)cfg.seed;
  ...
  IF_MAIN_PROC { printf("threads=%lu ", thread_num); }
...
#pragma omp parallel num_threads((int)thread_num) default(none)                \
    shared(cfg, domain, sharks, scratch, best_pos, best_min, R1, R2, r3, best, \
               best_r3, seed, rank, size)
      {
...
#pragma omp for schedule(static)
            for (size_t dim = 0; dim < cfg.nd; ++dim) {
              ...
              double derivative =
                  eval_derivative(shark_ptr->position, cfg.nd, cfg.obj, dim);
...
#pragma omp for schedule(static)
            for (size_t dim = 0; dim < cfg.nd; ++dim) {
              shark_ptr->position[dim] =
                  utils_clamp(shark_ptr->position[dim] + shark_ptr->speed[dim],
                              &domain[dim]);
            }
...
#pragma omp for schedule(static)
              for (size_t dim = 0; dim < cfg.nd; ++dim) {
                scratch[dim] = utils_clamp(shark_ptr->position[dim] * (1 + r3),
                                           &domain[dim]);
              }
...
#pragma omp for schedule(static)
              for (size_t dim = 0; dim < cfg.nd; ++dim) {
                shark_ptr->position[dim] = utils_clamp(
                    shark_ptr->position[dim] * (1 + best_r3), &domain[dim]);
              }
            }
...
#pragma omp single
      ...
      MPI_Allreduce(&local_best, &global_best, 1, MPI_DOUBLE_INT, MPI_MINLOC,
                    MPI_COMM_WORLD);

      if (rank == MAIN_RANK) {
        if (global_best.rank != MAIN_RANK) {
          MPI_Recv(best_pos, cfg.nd, MPI_DOUBLE, global_best.rank, MSG_POS,
                   MPI_COMM_WORLD, NULL);
        }
        best_min = global_best.val;
      } else if (rank == global_best.rank) {
        MPI_Send(best_pos, cfg.nd, MPI_DOUBLE, MAIN_RANK, MSG_POS,
                 MPI_COMM_WORLD);
      }
    ...
    IF_MAIN_PROC { print_result(best_min, best_pos, cfg.nd); }
...
#define RET_PARSE_BOUNDED(size, args, field, val, min, max, ...)               \
  do {                                                                         \
    char *end;                                                                 \
...
#define RET_PARSE(size, args, field, val, ...)                                 \
  do {                                                                         \
    char *end;                                                                 \
    (args)->field = strto##size((val), &end __VA_OPT__(, ) __VA_ARGS__);       \
    if (*(val) == 0 || *end != 0) {                                            \
